...
function initAudio() {
  if (!audioCtx) {
    try {
      ...
      console.warn("Áudio não suportado neste navegador mobile:", e);
    }
  }
}
...
function initFirebase() {
  try {
    ...
    console.warn("Firebase init blockado ou já ativo:", e);
  }
...
function calcPoints(elapsedMs, streak) { 
  const safeDuration = typeof QUESTION_MS !== 'undefined' ? QUESTION_MS : 20000;
  const clamped = Math.max(0, Math.min(safeDuration, elapsedMs)); 
  const ratio = 1 - (clamped / safeDuration); 
...
function startLeafFall() { if (!leafInterval) leafInterval = setInterval(spawnLeafFall, 300); }
...
/* ============== NAV ============== */
function goLanding() { role = null; show("screen-landing"); startAmbientLeaves(); }
...
async function goHostSetup() {
  try {
    ...
    console.warn("Ignorando áudio para evitar travamento", e);
  }
  role = "host";
  ...
  code = roomCode();
  const safeDuration = typeof QUESTION_MS !== 'undefined' ? QUESTION_MS : 20000;
  const safeTotal = typeof QUESTIONS !== 'undefined' ? QUESTIONS.length : 20;
  ...
  if (metaR) metaR.on("value", snap => { const m = snap.val(); if (m) localMeta = m; });
}
...
function renderLobbyPlayers() {
  ...
  if (btn) btn.disabled = false; // Removida a trava. Agora você pode testar sozinho sem ninguém na sala!
}

async function hostStartGame() {
  try { initAudio(); sfxStart(); } catch(e) {}
...
function listenForAnswers() {
  ...
  if (ansRef) {
    ansRef.on("value", snap => {
      const val = snap.val() || {};
      let count = 0; for (const k in val) count++;
      let total = 0; for (const k in hostRoster) total++;
...
async function hostReveal() {
  ...
  for (const key in answers) { const a = answers[key]; if (a && a.choice >= 0 && a.choice < 4) counts[a.choice]++; }
  let total = 0; for (const k in answers) total++; total = Math.max(1, total);

  for (let i = 0; i < qd.opts.length; i++) {
    const el = document.getElementById("host-opt-" + i); if (!el) continue;
    if (i === qd.c) el.classList.add("correct"); else el.classList.add("dim");
    const pct = Math.round((counts[i] / total) * 100);
    el.innerHTML += `<div class="bar" style="width:${pct}%"></div><div class="bar-label">${pct}%</div>`;
  }

  for (const key in answers) {
    const a = answers[key]; if (!a || !a.pid) continue;
    const isCorrect = a.choice === qd.c;
    const pRef = dbRef(dbPath("broto", code, "players", a.pid));
    if (pRef) {
      const pSnap = await pRef.get(); const p = pSnap.val() || {};
      const pts = isCorrect ? calcPoints(a.at - localMeta.qStartAt, p.streak || 0).total : 0;
      p.score = (p.score || 0) + pts;
      if (isCorrect) { p.streak = (p.streak || 0) + 1; p.bestStreak = Math.max(p.bestStreak || 0, p.streak); } else { p.streak = 0; }
      await pRef.set(p);
    }
  }
...
async function hostNext() {
  if (localMeta.qIndex + 1 < QUESTIONS.length) {
    ...
    const metaRef = dbRef(dbPath("broto", code, "meta")); if (metaRef) await metaRef.set(localMeta);
    renderHostQuestion(); startHostTimerTick(); listenForAnswers(); updateLivePodium();
  } else {
...
async function updateLivePodium() {
  ...
  const items = []; const val = snap.val() || {};
  for (const k in val) if (val[k]) items.push(val[k]);
  items.sort((a, b) => (b.score || 0) - (a.score || 0));

  const container = document.getElementById("live-podium-content"); if (!container) return;
  let maxScore = 1; for (let i = 0; i < items.length; i++) maxScore = Math.max(maxScore, items[i].score || 0);

  let html = "";
  for (let i = 0; i < Math.min(items.length, 5); i++) {
    const p = items[i]; const isLeader = i === 0; const pct = ((p.score || 0) / maxScore) * 100;
    const avIdx = AVATAR_DATA.findIndex(d => d.name === p.avatar); const safeIdx = avIdx >= 0 ? avIdx : 0;
    const avUrl = getAvatarUrl(safeIdx, 36); const fallback = getFallbackAvatar(safeIdx, 36);
    html += `<div class="live-podium-row ${isLeader ? 'leader' : ''}">
      <span class="lp-rank">#${i + 1}</span>
      <img class="lp-av" src="${avUrl}" alt="" style="border-radius:50%;width:28px;height:28px;object-fit:cover;" onerror="this.src='${fallback}'">
...
async function renderHostPodium() {
  clearInterval(tickTimer); livePodiumVisible = false; document.getElementById("live-podium").style.display = "none";
  const playersRef = dbRef(dbPath("broto", code, "players"));
  const snap = playersRef ? await playersRef.get() : { val: () => null };
  const items = []; const val = snap.val() || {};
  for (const k in val) if (val[k]) items.push(val[k]);
  ...
  show("screen-host-podium"); startLeafFall();
  const stage = document.getElementById("host-podium"); stage.innerHTML = "";
  const medals = ["🥇", "🥈", "🥉"]; const flowers = ["🌻", "🌼", "🌸"];

  for (let idx = 0; idx < items.length; idx++) {
    const p = items[idx]; const isTop3 = idx < 3; const rank = idx + 1;
    const medal = isTop3 ? medals[idx] : ""; const flower = isTop3 ? flowers[idx] : "🌱";
    const avIdx = AVATAR_DATA.findIndex(d => d.name === p.avatar); const safeIdx = avIdx >= 0 ? avIdx : 0;
    const avUrl = getAvatarUrl(safeIdx, 56); const fallback = getFallbackAvatar(safeIdx, 56);
    const topClass = isTop3 ? (idx === 0 ? "top-1" : idx === 1 ? "top-2" : "top-3") : "";

    const step = document.createElement("div");
    step.className = "podium-step " + topClass; step.id = "podium-step-" + idx;
    step.innerHTML = `
      <div class="step-rank">#${rank}</div>
      <img class="step-av" src="${avUrl}" alt="${escapeHtml(p.name)}" style="border-radius:50%;width:52px;height:52px;object-fit:cover;" onerror="this.src='${fallback}'">
      <div class="step-info">
      ...
      <div class="step-bar-bg"></div><div class="step-bar-fill"></div>
    `;
    stage.appendChild(step);
  }

  const rl = document.getElementById("host-ranklist"); let rlHtml = "";
  for (let i = 0; i < items.length; i++) {
    const p = items[i];
    const avIdx = AVATAR_DATA.findIndex(d => d.name === p.avatar); const safeIdx = avIdx >= 0 ? avIdx : 0;
    const avUrl = getAvatarUrl(safeIdx, 28); const fallback = getFallbackAvatar(safeIdx, 28);
    rlHtml += `<div class="rank-row" style="animation-delay:${i * 0.08}s">
      <span class="rn">${i + 1}</span>
      <img src="${avUrl}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;" alt="" onerror="this.src='${fallback}'">
  ...
  podiumItems = items; podiumRevealIndex = items.length - 1;
  sfxDrumRoll(); setTimeout(revealNextPodiumStep, 800);
}
...
function revealNextPodiumStep() {
  if (podiumRevealIndex < 0) {
    setTimeout(() => {
      ...
      if (winnerStep) {
        winnerStep.classList.add("winner-glow", "crowned");
        sfxWinner(); spawnConfetti(); spawnGoldRain();
  ...
  if (step) {
    step.classList.add("revealed"); sfxReveal();
    setTimeout(() => {
      const bar = step.querySelector(".step-bar-fill");
      if (bar) {
        let maxScore = 1; for (let i = 0; i < podiumItems.length; i++) maxScore = Math.max(maxScore, podiumItems[i].score || 0);
        const pct = ((podiumItems[podiumRevealIndex].score || 0) / maxScore) * 100; bar.style.width = pct + "%";
      }
      const bgBar = step.querySelector(".step-bar-bg"); if (bgBar) bgBar.style.width = "100%";
    }, 150);
  }
  podiumRevealIndex--; const delay = podiumRevealIndex >= 2 ? 700 : 1100; setTimeout(revealNextPodiumStep, delay);
}

/* ============== PLAYER FLOW ============== */
function goJoinCode() { role = "player"; show("screen-join-code"); document.getElementById("join-code-error").textContent = ""; setTimeout(() => { const input = document.getElementById("join-code-input"); if (input) input.focus(); }, 300); }
async function submitCode() {
  ...
  const metaRef = dbRef(dbPath("broto", val, "meta")); const snap = metaRef ? await metaRef.get() : { val: () => null, exists: () => false };
  if (!snap.exists()) { document.getElementById("join-code-error").textContent = "Não achamos essa sala. Confira o código."; return; }

  code = val; localMeta = snap.val(); buildAvatarGrid(); show("screen-join-profile");
  setTimeout(() => { const input = document.getElementById("join-name-input"); if (input) input.focus(); }, 300);
}

function buildAvatarGrid() {
  const g = document.getElementById("avatar-grid"); let html = "";
  for (let i = 0; i < AVATARS.length; i++) {
    const isSel = i === 0 ? " sel" : "";
    html += `<div class="avatar-opt${isSel}" data-idx="${i}" onclick="pickAvatar(this)">
      <img src="${AVATARS[i].url}" alt="${AVATARS[i].name}" loading="lazy" style="border-radius:50%;width:52px;height:52px;object-fit:cover;" onerror="this.src='${AVATARS[i].fallback}'">
      <div class="av-name">${AVATARS[i].name}</div>
    </div>`;
  }
  g.innerHTML = html; myAvatar = AVATARS[0];
}

function pickAvatar(el) { const opts = document.querySelectorAll(".avatar-opt"); opts.forEach(o => o.classList.remove("sel")); el.classList.add("sel"); myAvatar = AVATARS[parseInt(el.dataset.idx)]; }

async function playerJoinRoom() {
  const nameInput = document.getElementById("join-name-input"); const name = nameInput.value.trim();
  if (!name) { document.getElementById("join-profile-error").textContent = "Escreve um nome ou apelido pra gente te chamar :)"; return; }
  if (containsBadWord(name)) { document.getElementById("join-profile-error").textContent = "Esse nome não pode ser usado."; nameInput.classList.add("shake"); setTimeout(() => nameInput.classList.remove("shake"), 400); return; }
  
  myName = name.slice(0, 18); myPid = uid();
  const p = { pid: myPid, name: myName, avatar: myAvatar.name, avatarUrl: myAvatar.url, avatarPalette: myAvatar.palette, score: 0, streak: 0, bestStreak: 0, joinedAt: Date.now() };

  const pRef = dbRef(dbPath("broto", code, "players", myPid)); if (pRef) await pRef.set(p);
  const avImg = document.getElementById("player-wait-av-img"); avImg.src = myAvatar.url; avImg.onerror = function() { this.src = myAvatar.fallback; };
  document.getElementById("player-wait-name").textContent = myName; document.getElementById("player-wait-code").textContent = "sala " + code;
  show("screen-player-wait"); lastSeenStatus = "lobby"; lastSeenQIndex = -1;

  const metaRef = dbRef(dbPath("broto", code, "meta")); if (metaRef) metaRef.on("value", snap => { const meta = snap.val(); if (meta) handlePlayerMetaChange(meta); });
}

function handlePlayerMetaChange(meta) {
  localMeta = meta;
  if (meta.status === "question" && (lastSeenStatus !== "question" || meta.qIndex !== lastSeenQIndex)) {
    lastSeenStatus = "question"; lastSeenQIndex = meta.qIndex; hasAnsweredThisQ = false; renderPlayerQuestion(); show("screen-player-question");
  } else if (meta.status === "reveal" && lastSeenStatus !== "reveal") {
    lastSeenStatus = "reveal"; renderPlayerReveal().then(() => show("screen-player-reveal"));
  } else if (meta.status === "podium" && lastSeenStatus !== "podium") {
    lastSeenStatus = "podium"; renderPlayerPodium().then(() => show("screen-player-podium"));
...
function renderPlayerQuestion() {
  try { initAudio(); } catch(e) {}
  const qd = typeof QUESTIONS !== 'undefined' ? QUESTIONS[localMeta.qIndex] : { q: "Erro de carregamento", opts: ["A","B","C","D"], c: 0 };
  document.getElementById("player-qcounter").textContent = "Pergunta " + (localMeta.qIndex + 1) + " de " + (typeof QUESTIONS !== 'undefined' ? QUESTIONS.length : 20);
  document.getElementById("player-qtext").textContent = qd.q; document.getElementById("player-answer-status").textContent = "";

  const streakBadge = document.getElementById("player-streak-badge"); const streakText = document.getElementById("player-streak-text");
  if (myStreak >= 2) { streakBadge.style.display = "inline-flex"; streakText.textContent = "Série x" + myStreak + " 🔥"; } else { streakBadge.style.display = "none"; }

  let optsHtml = "";
  for (let i = 0; i < qd.opts.length; i++) {
    optsHtml += `<div class="opt o${i} clickable" id="player-opt-${i}" onclick="playerAnswer(${i})">
      <span class="mk">${OPT_MARK[i]}</span><span>${qd.opts[i]}</span>
    </div>`;
  }
  document.getElementById("player-opts").innerHTML = optsHtml; updatePlayerTimer(); updatePlayerGap();
}
...
function updatePlayerTimer() {
  ...
  const pct = Math.max(0, Math.min(100, (remain / localMeta.duration) * 100));
  const bar = document.getElementById("player-timerbar"); const lab = document.getElementById("player-timer");
  if (bar) bar.style.width = pct + "%"; if (lab) lab.textContent = Math.ceil(remain / 1000) + "s";
}

async function updatePlayerGap() {
  if (!code || !myPid || gapUpdatePending) return; gapUpdatePending = true;
  try {
    const playersRef = dbRef(dbPath("broto", code, "players")); const snap = playersRef ? await playersRef.get() : { val: () => null };
    const items = []; const val = snap.val() || {}; for (const k in val) if (val[k]) items.push(val[k]);
    items.sort((a, b) => (b.score || 0) - (a.score || 0));

    let myRank = -1, myScore = 0; for (let i = 0; i < items.length; i++) { if (items[i].pid === myPid) { myRank = i + 1; myScore = items[i].score || 0; } }
    const gapEl = document.getElementById("player-gap-info"); const gapText = document.getElementById("player-gap-text");

    if (myRank > 1 && items[myRank - 2]) {
      const gap = (items[myRank - 2].score || 0) - myScore; gapEl.style.display = "inline-flex"; gapText.textContent = "Você está a " + gap + " pts do " + (myRank - 1) + "º lugar";
    } else if (myRank === 1 && items[1]) {
      const lead = myScore - (items[1].score || 0); gapEl.style.display = "inline-flex"; gapText.textContent = "Liderando por " + lead + " pts 🔥";
    } else { gapEl.style.display = "none"; }
  } finally { gapUpdatePending = false; }
}

setInterval(() => { if (role === "player" && document.getElementById("screen-player-question").classList.contains("active")) { updatePlayerTimer(); updatePlayerGap(); } }, 800);

async function playerAnswer(idx) {
  if (hasAnsweredThisQ) return; hasAnsweredThisQ = true;
  const opts = document.querySelectorAll("#player-opts .opt");
  opts.forEach((opt, i) => { opt.classList.remove("clickable"); if (i !== idx) opt.classList.add("dim"); });
...
async function renderPlayerReveal() {
  const qd = typeof QUESTIONS !== 'undefined' ? QUESTIONS[localMeta.qIndex] : { c: 0 }; 
  const ansRef = dbRef(dbPath("broto", code, "answers", localMeta.qIndex, myPid));
  const myAns = ansRef ? (await ansRef.get()).val() : null; const pRef = dbRef(dbPath("broto", code, "players", myPid));
  const p = pRef ? (await pRef.get()).val() : null; const isCorrect = myAns && myAns.choice === qd.c;

  if (isCorrect) { myStreak++; myBestStreak = Math.max(myBestStreak, myStreak); sfxStreak(myStreak); } else { myStreak = 0; }
  const banner = document.getElementById("player-reveal-banner"); banner.className = "reveal-banner " + (isCorrect ? "ok" : "no");
  ...
  document.getElementById("player-reveal-points").textContent = isCorrect ? ("+" + pts.total + " pontos" + (pts.bonus > 0 ? " (" + pts.base + " + 🔥" + pts.bonus + ")" : "")) : "";
  document.getElementById("player-reveal-total").textContent = "Pontuação total: " + (p ? p.score || 0 : 0);

  const streakReveal = document.getElementById("player-streak-reveal"); const streakRevealText = document.getElementById("player-streak-reveal-text");
  if (myStreak >= 2) { streakReveal.style.display = "flex"; streakRevealText.textContent = "Série de " + myStreak + " acertos! 🔥"; } else { streakReveal.style.display = "none"; }
  ...
  let myRank = -1, myScore = 0; for (let i = 0; i < items.length; i++) { if (items[i].pid === myPid) { myRank = i + 1; myScore = items[i].score || 0; } }
  const gapRevealEl = document.getElementById("player-gap-reveal"); const gapRevealText = document.getElementById("player-gap-reveal-text");
  if (myRank > 1 && items[myRank - 2]) {
    const gap = (items[myRank - 2].score || 0) - myScore; gapRevealEl.style.display = "inline-flex"; gapRevealText.textContent = "Você está a " + gap + " pts do " + (myRank - 1) + "º lugar";
  } else if (myRank === 1 && items[1]) {
    const lead = myScore - (items[1].score || 0); gapRevealEl.style.display = "inline-flex"; gapRevealText.textContent = "Liderando por " + lead + " pts 🔥";
  } else { gapRevealEl.style.display = "none"; }
}
...
async function renderPlayerPodium() {
  ...
  let myRank = -1; let me = null; for (let i = 0; i < items.length; i++) { if (items[i].pid === myPid) { myRank = i + 1; me = items[i]; } }
  const emojis = ["🥇", "🥈", "🥉"]; document.getElementById("player-final-emoji").textContent = myRank <= 3 && myRank > 0 ? emojis[myRank - 1] : "🌱";
  ...
  let listHtml = "";
  for (let i = 0; i < items.length; i++) {
    const p = items[i]; const isMe = p.pid === myPid;
    const avIdx = AVATAR_DATA.findIndex(d => d.name === p.avatar); const safeIdx = avIdx >= 0 ? avIdx : 0;
    const avUrl = getAvatarUrl(safeIdx, 28); const fallback = getFallbackAvatar(safeIdx, 28);
    ...
    listHtml += `<div class="rank-row" style="animation-delay:${i * 0.08}s;${highlight}">
      <span class="rn">${i + 1}</span><img src="${avUrl}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;" onerror="this.src='${fallback}'">
      <span>${escapeHtml(p.name)}</span><span class="rs">${p.score || 0} pts${p.bestStreak > 2 ? ' 🔥' + p.bestStreak : ''}</span>
    </div>`;
  }
  document.getElementById("player-final-list").innerHTML = listHtml;
  show("screen-player-podium"); startLeafFall();
  if (myRank <= 3 && myRank > 0) { sfxWinner(); spawnConfetti(); spawnGoldRain(); setTimeout(() => { spawnConfetti(); spawnGoldRain(); }, 400); }
}

/* ============== BOOT ============== */
(function boot() {
  const params = new URLSearchParams(location.search); const preCode = params.get("code");
  if (preCode && /^\d{6}$/.test(preCode)) {
    role = "player"; if (!initFirebase()) enableDemoMode();
    (async () => {
      const metaRef = dbRef(dbPath("broto", preCode, "meta")); const snap = metaRef ? await metaRef.get() : { val: () => null, exists: () => false };
      if (snap.exists()) { code = preCode; localMeta = snap.val(); buildAvatarGrid(); show("screen-join-profile"); } else { goLanding(); }
    })();
  } else { goLanding(); }
})();  size = size || 120;
  return `https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(seed)}&size=${size}&backgroundColor=b6e3f4,c0aede,d1d4f9`;
...
function roomCode() { return String(Math.floor(100000 + Math.random() * 900000)); }
function escapeHtml(s) { const div = document.createElement('div'); div.textContent = s || ""; return div.innerHTML; }
function calcPoints(elapsedMs, streak) { const clamped = Math.max(0, Math.min(QUESTION_MS, elapsedMs)); const ratio = 1 - (clamped / QUESTION_MS); const base = Math.round(500 + 500 * ratio); const streakBonus = Math.min(streak, 10) * 0.10; const total = Math.round(base * (1 + streakBonus)); return { base, bonus: Math.round(base * streakBonus), total }; }
...
function goJoinCode() { role = "player"; show("screen-join-code"); document.getElementById("join-code-error").textContent = ""; setTimeout(() => { const input = document.getElementById("join-code-input"); if (input) input.focus(); }, 300); }
async function submitCode() {
  initAudio(); const val = document.getElementById("join-code-input").value.replace(/\D/g, "");
  if (val.length !== 6) { document.getElementById("join-code-error").textContent = "Digite os 6 números do código."; return; }
  if (!initFirebase()) enableDemoMode();
...
function renderPlayerQuestion() {
  initAudio(); const qd = QUESTIONS[localMeta.qIndex];
...
async function renderPlayerReveal() {
  const qd = QUESTIONS[localMeta.qIndex]; const ansRef = dbRef(dbPath("broto", code, "answers", localMeta.qIndex, myPid));
  const myAns = ansRef ? (await ansRef.get()).val() : null; const pRef = dbRef(dbPath("broto", code, "players", myPid));
  const p = pRef ? (await pRef.get()).val() : null; const isCorrect = myAns && myAns.choice === qd.c;
...
function normalizeLeet(text) {
  ...
  // split e join nativo evita quebras com sintaxes complexas de RegExp
  replacements.forEach(([from, to]) => {
    result = result.split(from).join(to);
  });
  return result;
...
function containsBadWord(name) {
  ...
  // Validação segura para repetição consecutiva de letras
  if (/(.)\1{5,}/.test(lower)) return true;
  if (/^\d+$/.test(lower)) return true;
...
// ... Mantém as funções utilitárias restantes (uid, roomCode, escapeHtml, calcPoints) identicas ...
function uid() { return "p" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8); }
if (typeof roomCode !== "function") { function roomCode() { return String(Math.floor(100000 + Math.random() * 900000)); } }
function escapeHtml(s) { const div = document.createElement('div'); div.textContent = s || ""; return div.innerHTML; }
function calcPoints(elapsedMs, streak) { const clamped = Math.max(0, Math.min(QUESTION_MS, elapsedMs)); const ratio = 1 - (clamped / QUESTION_MS); const base = Math.round(500 + 500 * ratio); const streakBonus = Math.min(streak, 10) * 0.10; const total = Math.round(base * (1 + streakBonus)); return { base, bonus: Math.round(base * streakBonus), total }; }
